...
void EffReco(string fName = "nope.root",string outFile="save.root"){
    UInt_t stub=0;
    ...
    for (int i =0; i<t->GetEntries(); i++){
        ...
        if (cl0.size()==1){
            ...
            for (int j=0; j<cl1.size(); j++) if (abs(cl0.at(0).x-cl1.at(j).x+effOffset)<effWindow+1){effPos0->Fill(cl0.at(0).x);nEffCandi0++;break;}
        }
