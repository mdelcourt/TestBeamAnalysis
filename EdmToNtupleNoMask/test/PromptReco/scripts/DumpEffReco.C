...
int effOffset = 0;
int effWindow= 2 ; // will look for cluster in offset +- effWidth
...
void DumpEffReco(string fName = "nope.root",string outFile="save.root"){
    UInt_t stub=0;
    ...
    int N30(0),N31(0),P30(0),P31(0),N40(0),N41(0),P40(0),P41(0);
    int N[16];
    int P[16];
    for (int i=0; i<16; i++){N[i]=0; P[i]=0;}
    int nHits = 0;
    int nEvent= 0;
    int nClust =0;
    int clSize=0;
    ...
    for (int i =0; i<t->GetEntries(); i++){
        ...
        if (cl0.size()==1){
            if (cl0.at(0).x<CBCSIZE*4 && cl0.at(0).x >CBCSIZE*3){N30++; N[tdc]++;}
            else if (cl0.at(0).x<CBCSIZE*5 && cl0.at(0).x >CBCSIZE*4){N40++;}
            for (int j=0; j<cl1.size(); j++) if (abs(cl0.at(0).x-cl1.at(j).x+effOffset)<effWindow+1){
                   if (cl0.at(0).x<CBCSIZE*4 && cl0.at(0).x >CBCSIZE*3){P30++; P[tdc]++;}
                   else if (cl0.at(0).x<CBCSIZE*5 && cl0.at(0).x >CBCSIZE*4){P40++;}
                   break;
                }
        }

        if (cl1.size()==1){
            if (cl1.at(0).x<CBCSIZE*4 && cl1.at(0).x >CBCSIZE*3){N31++;}
            if (cl1.at(0).x<CBCSIZE*5 && cl1.at(0).x >CBCSIZE*4){N41++;}
            for (int j=0; j<cl0.size(); j++) if (abs(cl1.at(0).x-cl0.at(j).x+effOffset)<effWindow+1){
                   if (cl1.at(0).x<CBCSIZE*4 && cl1.at(0).x >CBCSIZE*3){P31++;}
                   else if (cl1.at(0).x<CBCSIZE*5 && cl1.at(0).x >CBCSIZE*4){P41++;}
                   break;
                }
        }
//        if (cl1.size()==1){
